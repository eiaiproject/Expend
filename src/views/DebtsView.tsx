    ...
    <div className="p-4 space-y-6 pb-24">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--text-secondary)]">Catatan tersimpan di perangkat ini.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            aria-label="Catat Utang / Piutang"
          >
            <Plus size={20} />
          </button>
        </div>
      ...
      <div className="rounded-[16px] border border-[var(--border)] bg-[var(--card)] p-4">
        <div className="mb-4 flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">Tidak termasuk saldo wallet</span>
        </div>
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-bold">Utang Saya</p>
              <p className="text-xs text-[var(--text-secondary)]">Uang yang masih perlu kamu bayar</p>
            </div>
            <p className="shrink-0 font-mono font-bold text-amber-500">{formatCurrency(summary.payableTotal, hideAmount)}</p>
          </div>
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-bold">Piutang Saya</p>
              <p className="text-xs text-[var(--text-secondary)]">Uang yang masih perlu kamu terima</p>
            </div>
          ...
          <div className="border-t border-[var(--border)] pt-3 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-bold">Posisi Bersih</p>
              <p className="text-xs text-[var(--text-secondary)]">Piutang - Utang</p>
            </div>
      ...
      {summary.attentionCount > 0 && (
        <div className="rounded-[16px] border border-red-500/20 bg-red-500/10 p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-bold text-red-500">Perlu Perhatian</h2>
              <p className="mt-1 text-sm text-[var(--text-secondary)]">
                {summary.overdueCount} lewat jatuh tempo
                {summary.dueSoonCount > 0 ? ` • ${summary.dueSoonCount} jatuh tempo minggu ini` : ''}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="relative group">
        ...
        <input
          type="text"
          aria-label="Cari nama atau catatan"
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center rounded-[16px] border border-[var(--border)] bg-[var(--card)] px-6 py-12 text-center">
          ...
          <h2 className="font-bold">{hasAnyDebt ? 'Tidak ada catatan yang cocok' : 'Belum ada utang atau piutang'}</h2>
          <p className="mt-2 max-w-[260px] text-sm text-[var(--text-secondary)]">
            {hasAnyDebt
              ? 'Coba ubah filter atau kata kunci pencarian.'
              : 'Catat pinjaman agar kamu tahu siapa yang harus dibayar dan siapa yang masih harus membayar kamu.'}
          </p>
          {!hasAnyDebt && (
            <button
              type="button"

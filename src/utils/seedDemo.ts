...
export async function seedDemoData(): Promise<number> {
  ...
  const rows: Array<Parameters<typeof db.transactions.bulkAdd>[0][number]> = [];
  let topupNo = 0;
  ...
  for (let d = new Date(START); d <= today; d.setDate(d.getDate() + 1)) {
    ...
    if (dayOfMonth === 25) {
      rows.push({ walletId: bcaId, categoryId: cat('Lainnya'), date, description: 'Gaji bulanan', type: 'balance_adjustment', amount: 8_500_000 });
      if (d.getMonth() % 2 === 0) {
        rows.push({ walletId: bcaId, categoryId: cat('Lainnya'), date, description: 'Proyek freelance', type: 'balance_adjustment', amount: ri(1_000_000, 2_500_000) });
      }
    }

    // Fixed bills on the 1st (skip the very first day's double — fine, rent counts)
    if (dayOfMonth === 1) {
      rows.push({ walletId: bcaId, categoryId: cat('Tagihan & Langganan'), date, description: 'Sewa Kos', type: 'expense', amount: 2_500_000 });
      rows.push({ walletId: bcaId, categoryId: cat('Tagihan & Langganan'), date, description: 'Listrik PLN', type: 'expense', amount: ri(300_000, 480_000) });
      rows.push({ walletId: bcaId, categoryId: cat('Tagihan & Langganan'), date, description: 'WiFi IndiHome', type: 'expense', amount: 350_000 });
      rows.push({ walletId: bcaId, categoryId: cat('Tagihan & Langganan'), date, description: 'Pulsa Telkomsel', type: 'expense', amount: 100_000 });
      rows.push({ walletId: gopayId, categoryId: cat('Hiburan'), date, description: 'Netflix', type: 'expense', amount: 79_000 });
      rows.push({ walletId: gopayId, categoryId: cat('Hiburan'), date, description: 'Spotify', type: 'expense', amount: 54_000 });
    }
    ...
    if (dayOfMonth === 5 || dayOfMonth === 15 || dayOfMonth === 25) {
      ...
      const groupId = `seed-topup-${++topupNo}`;
      rows.push({ walletId: bcaId, categoryId: null, date, description: 'Top-up GoPay', type: 'transfer_out', amount, transferGroupId: groupId });
      rows.push({ walletId: gopayId, categoryId: null, date, description: 'Top-up GoPay', type: 'transfer_in', amount, transferGroupId: groupId });
    }

    // Daily food: 1–3 meals
    const meals = ri(1, 3);
    for (let m = 0; m < meals; m++) {
      const wallet = rand() < 0.08 ? cashId : rand() < 0.72 ? gopayId : bcaId;
      rows.push({
        walletId: wallet,
        categoryId: cat('Makanan & Minuman'),
        date,
    ...
    if (!isWeekend && rand() < 0.6) {
      rows.push({ walletId: gopayId, categoryId: cat('Transportasi'), date, description: pick(TRANSPORT_PAYEES), type: 'expense', amount: ri(3_500, 45_000) });
    }

    // Weekend leisure/shopping
    if (isWeekend && rand() < 0.45) {
      if (rand() < 0.5) {
        rows.push({ walletId: gopayId, categoryId: cat('Belanja'), date, description: pick(SHOP_PAYEES), type: 'expense', amount: ri(50_000, 500_000) });
      } else {
        rows.push({ walletId: gopayId, categoryId: cat('Hiburan'), date, description: pick(FUN_PAYEES), type: 'expense', amount: ri(25_000, 150_000) });
      }
    }

    // Occasional health
    if (rand() < 0.03) {
      rows.push({ walletId: bcaId, categoryId: cat('Kesehatan'), date, description: pick(HEALTH_PAYEES), type: 'expense', amount: ri(30_000, 250_000) });
    }
  }

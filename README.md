...
![version](https://img.shields.io/badge/version-0.15.9-teal)
...
Upload a photo of the transfer receipt → auto-detected → edit if needed → Save. On shop receipts, `Total` beats cash tendered and change (`Total Rp 55.000` wins over `Tunai Rp 100.000` / `Kembalian Rp 45.000`). On QRIS merchant receipts the store name is used as description (`TYA BUAH 2`, not the acquirer bank).
